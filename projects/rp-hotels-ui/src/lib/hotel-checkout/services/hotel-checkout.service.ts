...
export class HotelCheckoutService {
  ...
  loadDataCard(
    ...
    this.subscription.add(
      this.api
        .GetHotelRooms(providerId, searchId, HotelCode)
        .subscribe((res) => {
          if (res == undefined) {
            return;
          } else {
            if (res) {
              ...
              this.HotelResult = HotelPackage.filter(
                (v) => v.PackageKey === packageKey,
              )[0].Rooms;
  ...
  FormRooms() {
    for (var i = 0; i < this.roomLength; i++) {
      if (this.HotelResult[0].Adult != 0 && !this.firstAdultFound) {
        ...
        (<FormArray>this.HotelForm.get('Travellers')).push(
          new FormGroup({
  ...
  applyCopounCode(
    ...
    this.subscription.add(
      this.api
        .activateCobon(copounCode, searchId, packageKey, providerId)
        .subscribe(
          (res) => {
            if (res) {
              ...
              if (this.HotelResult) {
                ...
                this.HotelResult[0].TotalSellPrice -=
  ...
  prepareData(r: hotelRoomsResponse) {
    {
      ...
      for (var i = 0; i < r.Packages[0].Rooms.length; i++) {
        ...
        this.totalCostPrice += this.HotelResult[i].CostPrice
          ? this.HotelResult[i].CostPrice
          : 0;
      }

      this.HotelForm.get('sellPrice')?.setValue(
        parseFloat((Math.round(this.totalSellPrice * 100) / 100).toFixed(2)),
      );
      ...
      let phoneNumberObject: any = {
        ...(<FormArray>this.HotelForm.get('Travellers')).at(0).get('phonenum')
          ?.value,
      };
      ...
      for (
        var i = 0;
        i < (<FormArray>this.HotelForm.get('Travellers')).length;
        i++
      ) {
        (<FormArray>this.HotelForm.get('Travellers'))
          .at(i)
          .get('dateOfBirth')
        ...
        (<FormArray>this.HotelForm.get('Travellers'))
          .at(i)
          .get('roomRef')
          ?.setValue(this.packageKey);
        (<FormArray>this.HotelForm.get('Travellers'))
          .at(i)
          .get('roomNo')
          ?.setValue(this.HotelResult[i].RoomIndex);
        if (i > 0) {
          (<FormArray>this.HotelForm.get('Travellers'))
            .at(i)
            .get('dateOfBirth')
