...
export class HotelResultsService {
      ...
      this.api.getHotelsRes(hotelSearchObj).subscribe(
        (res) => {
          if (res) {
            ...
            this.minPrice =
              this.filteredHotels[this.filteredHotels.length - 1].costPrice;
            this.minPriceValueForSlider =
              this.filteredHotels[this.filteredHotels.length - 1].costPrice;
  ...
  generateSearchRooms(guestInfo: string) {
    ...
    for (let i = 0; i < roomsInfo.length; i++) {
      ...
      if (childNum === 0) {
        //If child Number 0 then send empty array
        age = [];
      } else if (childNum === 1) {
        //If Child Nuber 1 then send age is 7
        age = [7];
      } else if (childNum === 2) {
        //If Child Nuber 1 then send ages are 7 and 6
        age = [7, 6];
      }
  ...
  filterHotelData(hotel: hotel) {
    ...
    return (
      ...
      hotel.costPrice >= hotelPrice &&
      hotel.costPrice <= this.maxPrice &&
      this.locationsArrSelected.includes(hotel.City) &&
