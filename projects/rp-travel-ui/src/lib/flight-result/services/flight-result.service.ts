...
export class FlightResultService {
  ...
  getDataFromUrl(
    ...
    this.formINIT = false;
    if (
      this.flightSearch.searchFlight.get("flightType")?.value == "RoundTrip"
    ) {
      this.roundT = true;
    } else {
      this.roundT = false;
      ...
      this.filterForm.valueChanges.subscribe((val) => {
        if (this.formINIT) {
          let filter: flightResultFilter = new flightResultFilter(
            ...
            [
              this.filterForm.get("flexibleTickets")?.get("refund")?.value!,
              this.filterForm.get("flexibleTickets")?.get("nonRefund")?.value!,
            ],
  ...
  sortMyResult(type: number) {
    if (this.response != undefined) {
      if (type == 1) {
        this.orgnizedResponce = [...this.orgnizedResponce].sort((a, b) => {
          return a[0].itinTotalFare.amount - b[0].itinTotalFare.amount;
        });
      }
      if (type == 2) {
        this.orgnizedResponce = [...this.orgnizedResponce].sort((a, b) => {
          return a[0].totalDuration - b[0].totalDuration;
        });
      }
      if (type == 3) {
        this.orgnizedResponce = [...this.orgnizedResponce].sort((a, b) => {
          return <any>new Date(a[0].deptDate) - <any>new Date(b[0].deptDate);
        });
      }
      if (type == 4) {
        this.orgnizedResponce = [...this.orgnizedResponce].sort((a, b) => {
          return <any>new Date(b[0].deptDate) - <any>new Date(a[0].deptDate);
        });
      }
      if (type == 5) {
        this.orgnizedResponce = [...this.orgnizedResponce].sort((a, b) => {
          return (
            <any>(
              new Date(a[0].allJourney.flights[1].flightDTO[0].departureDate)
            ) -
            <any>new Date(b[0].allJourney.flights[1].flightDTO[0].departureDate)
          );
        });
      }
      ...
      if (type == 6) {
        this.orgnizedResponce = [...this.orgnizedResponce].sort((a, b) => {
          return (
            <any>(
              new Date(b[0].allJourney.flights[1].flightDTO[0].departureDate)
            ) -
            <any>new Date(a[0].allJourney.flights[1].flightDTO[0].departureDate)
          );
        });
      }
      ...
      if (type == 7) {
        this.orgnizedResponce = [...this.orgnizedResponce].sort((a, b) => {
          return a[0].experiance - b[0].experiance;
        });
      }
    }
  }
  ...
  fetchLowestFaresForSorting(data: airItineraries[][]) {
    ...
    this.cheapeastLowestFare = [...data].sort((a, b) => {
      return a[0].itinTotalFare.amount - b[0].itinTotalFare.amount;
    })[0][0].itinTotalFare.amount;
    this.bestExperienceLowestFare = [...data].sort((a, b) => {
      return a[0].experiance - b[0].experiance;
    })[0][0].itinTotalFare.amount;
    this.bestExperienceFlight = [...data].sort((a, b) => {
      return a[0].experiance - b[0].experiance;
    })[0][0];
    this.shortestLowestFare = [...data].sort((a, b) => {
      return a[0].totalDuration - b[0].totalDuration;
    })[0][0].itinTotalFare.amount;
    this.shortestFlight = [...data].sort((a, b) => {
      return a[0].totalDuration - b[0].totalDuration;
    })[0][0];
  }
  ...
  filterFlighWithDuration(flight: airItineraries): boolean {
    return (
      flight.totalDuration >=
        this.filterForm.get("mindurationSlider")?.value! &&
      flight.totalDuration < this.filterForm.get("maxdurationSlider")?.value!
    );
  }
  /**
  ...
  stopsvalues() {
    ...
    if (
      !this.filterForm.get("stopsForm")?.get("noStops")?.value &&
      !this.filterForm.get("stopsForm")?.get("oneStop")?.value &&
      !this.filterForm.get("stopsForm")?.get("twoAndm")?.value
    ) {
      out = [0, 1, 2, 3, 4];
    }
  ...
  filterFlightWithNumberofStopsFunction(
    ...
    if (
      ...
      for (var i = 0; i < flight.allJourney.flights.length; i++) {
        if (
          flight.allJourney.flights[i].stopsNum < 2 &&
          flight.allJourney.flights[i].stopsNum != 1
        ) {
          stopFlage = false;
        }
      }
  ...
  completeTripOnSameAirline(
    ...
    if (!filter.sameAirline) {
      ...
      let firstAirline =
        flight.allJourney.flights[0].flightDTO[0].flightAirline.airlineName;
  ...
  nextcustomFilteredAirlineMobile() {
    if (
      this.customFilteredAirlineEndMobile == this.customFilteredAirline.length
    ) {
      return;
    } else {
  ...
  calcExperiance(flightObj: airItineraries): number {
    return (
      ...
      this.calcOverNight(flightObj)
    ); //Flight Price + Flight Total Stops + Flight over night number
  }
  /**
   *
   */
  ...
  addStopTime(flights: flight[]): number {
    ...
    flights.forEach((flight) => {
      let transitTime = flight.flightDTO
        .map((f) => f.transitTime.split(":").map((t) => parseInt(t)))
        .reduce((ac, v) => {
