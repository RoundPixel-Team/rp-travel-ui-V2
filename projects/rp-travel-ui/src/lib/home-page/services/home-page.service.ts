...
export class HomePageService {
  ...
  currencyLoaded$ = new BehaviorSubject<currencyModel[]>([]);
  ...
  getAirports(currentLang: string, searchStr: string) {
    this.loader = true;
    this.subscription.add(
      this.api.UtilityAirports(currentLang, searchStr).subscribe(
        (res: airPorts[]) => {
          if (res) {
 ...
 */
  ...
  getAllOffers(pos: string) {
    this.loader = true;
    this.subscription.add(
      this.api.GetAllOffers(pos).subscribe(
        (res) => {
          if (res) {
  ...
  extractOfferData(id: number | string) {
    this.offerImages = this.selectedOffer.offerImage
      ? [this.selectedOffer.offerImage.url]
      : [];
    ...
    this.selectedOffer.offerServices.map((offerService) => {
      if (offerService.serviceType == '1') {
        this.subscription.add(
          this.api.retriveItinerary(offerService.offlineItinerary).subscribe(
            (res: Itinerary) => {
              if (res) {
  ...
  bookOffer(source: string, langCode: string, phonecountrycode: string) {
    ...
    if (this.offerCheckOutForm.valid) {
      ...
      this.subscription.add(
        this.api.BookOffers(source, langCode!, Body, offerId!).subscribe(
          (res: BookedOffer) => {
            if (res) {
