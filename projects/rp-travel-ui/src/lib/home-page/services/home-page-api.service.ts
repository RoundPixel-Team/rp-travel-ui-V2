...
export class HomePageApiService {
  ...
  UtilityAirports(lang: string): Observable<airPorts[]> {
    ...
    return this.http.get<airPorts[]>(API).pipe(
      retry(3),
      take(1),
      catchError((err) => {
        console.error(err);
  ...
  currencyApi(baseCurrency: string): Observable<currencyModel[]> {
    ...
    return this.http.get<currencyModel[]>(API).pipe(
      retry(3),
      take(1),
      catchError((err) => {
        console.error(err);
  ...
  getCountries(lang: string) {
    ...
    return this.http.get<countries[]>(api).pipe(
      retry(2),
      take(1),
      catchError((err) => {
        console.error(err);
  ...
  GetAllOffers(pos: string): Observable<{ offers: OfferDTO[] }> {
    ...
    return this.http.get<{ offers: OfferDTO[] }>(API).pipe(
      ...
      retry(3),
      catchError((err) => {
        console.error(err, 'ERROR IN GETTING ALL OFFERS');
        throw err;
      }),
    );
  }
  ...
  getOfferBYId(id: number | string): Observable<OfferDTO> {
    ...
    return this.http.get<OfferDTO>(API).pipe(
      ...
      catchError((err) => {
        console.error(err, 'ERROR IN GETTING OFFER BY ID');
        throw err;
      }),
    );
  }
  ...
  retriveItinerary(id: number | string) {
    ...
    return this.http.get<Itinerary>(API).pipe(
      retry(3),
      take(1),
      catchError((err) => {
        console.error(err);
