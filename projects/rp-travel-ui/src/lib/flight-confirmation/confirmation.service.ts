...
export class ConfirmationService {
  ...
  getConfirmationDate(searchId: string, hgNum: string, tok: string) {
    ...
    this.api.getConfirmation(hgNum, searchId, tok).subscribe(
      (res) => {
        if (res) {
  ...
  formatWegoClicktUrl() {
    ...
    let commission = total_booking_value * 0.02;
    let status = 'confirmed';
    if (localStorage.getItem('click_id')) {
      var url = `https://srv.wego.com/genzo/v2/conversions?conversion_id=c-wego-ejazzatravel.com&click_id=${localStorage.getItem(
        'click_id'
      )}&comm_currency_code=${comm_currency_code}&bv_currency_code=${bv_currency_code}&transaction_id=${transaction_id}&commission=${commission}&total_booking_value=${total_booking_value}&status=${status}`;
    } else {
      var url = `https://srv.wego.com/analytics/v2/conversions?conversion_id=c-wego-ejazzatravel.com&click_id=${'no_click_id'}&comm_currency_code=${comm_currency_code}&bv_currency_code=${bv_currency_code}&transaction_id=${transaction_id}&commission=${commission}&total_booking_value=${total_booking_value}&status=${status}`;
    }
