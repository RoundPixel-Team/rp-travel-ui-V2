...
export class FlightCheckoutApiService {
  ...
  oldSaveBooking(
    ...
    let api = `${this.env.BookingFlow}/api/SaveBooking?SearchId=${SearchId}&SeqNum=${SeqNum}&PKey=${PKey}&sCode=${sCode}`;
    return this.http.post<any>(api, body.checkOutDetails).pipe(
      take(1),
      retry(1),
      catchError((err) => {
        console.error(err);
  ...
  checkFlightValidation(
    ...
    let api = `${this.env.BookingFlow}/api/CheckFlightValidation?PKey=${PKey}&SeqNum=${SeqNum}&HGNum=${HGNum}&SearchId=${SearchId}`;
  ...
  preProcessing(SId: string, HG: string) {
    let api = `${this.env.BookingFlow}/api/PreProcessing?SId=${SId}&HG=${HG}`;
    return this.http.get(api).pipe(
      take(1),
      retry(1),
      catchError((err) => {
  ...
  mergePayment(
    ...
    charges.forEach((charge) => {
      let gate = gates.filter(
        (val) =>
          val?.GatewayType?.trim().toLowerCase() ===
          charge?.paymentMethod?.trim().toLowerCase(),
      )[0];
  ...
  addPaymentGateways(
    ...
    return this.http.post<any>(api, body).pipe(
      retry(3),
      map((val) => {
        return this.mergePayment(val, this.paymentGates);
      }),
      catchError((err) => {
        console.error(err);
        throw err;
  ...
  startPaymentProcess(
    ...
    if (src === 'mop' || src === '' || !src) {
      ...
      return this.http.get<paymnetdata>(api).pipe(
        retry(3),
        take(1),
        map((va) => {
          if (!va || va.Status != 0) {
      ...
      return this.http
        .post<paymnetdata>(this.env.prepay + '/api/startpaymentProcess', body)
        .pipe(
          retry(3),
          take(1),
          map((va) => {
