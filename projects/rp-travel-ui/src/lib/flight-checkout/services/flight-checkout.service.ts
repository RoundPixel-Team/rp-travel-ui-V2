...
export class FlightCheckoutService {
  ...
  getSelectedFlightData(
    ...
    this.subscription.add(
      this.api
        .getSelectedFlight(searchId, sequenceNum, providerKey, pcc, wegoClickId)
        .subscribe(
          (res: selectedFlight) => {
            if (res) {
  ...
  applyCouponCode(
    ...
    this.subscription.add(
      this.api
        .activateCobon(couponCode, searchId, sequenceNum, providerKey, pcc)
        .subscribe(
          (res) => {
            if (res) {
  ...
  removeCouponCode() {
    if (this.selectedFlight && this.couponCodeDetails) {
      this.selectedFlight.airItineraryDTO.itinTotalFare.amount +=
        this.couponCodeDetails.promotionDetails.discountAmount;
    }
