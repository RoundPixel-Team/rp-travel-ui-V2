...
@Injectable({
  ...
  getFlightDetails(flights: IFlightModel[]): ICardModel[] {
    return flights.map(flight => {
        ...
        const startDate = new Date(allFlights[0]?.flightDTO?.[0]?.departureDate).toLocaleDateString("en-GB", {
            year: 'numeric', month: 'long', day: 'numeric'
        });

        const endDate = flightType === "return" ? new Date(allFlights[allFlights.length - 1]?.flightDTO?.[0]?.arrivalDate).toLocaleDateString("en-GB", {
                year: 'numeric', month: 'long', day: 'numeric'
            })
            : null;

        // Determine the city image based on flight type
        const cityImage = flightType === "return"
